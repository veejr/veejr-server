defmodule VeejrWeb.UserAuth do
  ...
  @remember_me_cookie "_veejr_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      VeejrWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      VeejrWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule VeejrWeb.PageLive do
        use VeejrWeb, :live_view

        on_mount {VeejrWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{VeejrWeb.UserAuth, :require_authenticated}] do
  ...
  # the user was already logged in, redirect to contacts
  def signed_in_path(%Plug.Conn{assigns: %{current_scope: %Scope{user: %Accounts.User{}}}}) do
    ~p"/contacts"
  end

  def signed_in_path(_), do: ~p"/contacts"
